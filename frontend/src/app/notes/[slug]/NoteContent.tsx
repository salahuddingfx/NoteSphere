        ...
        <div className="grid gap-12 lg:grid-cols-3">
          <div className="lg:col-span-2 space-y-12">
            ...
            <div className="rounded-[2.5rem] border border-white/5 bg-white/5 p-10 backdrop-blur-xl">
              ...
              <p className="text-zinc-300 text-lg leading-relaxed whitespace-pre-wrap">{note.description}</p>
              {note.tags.length > 0 && (
                <div className="mt-8 flex flex-wrap gap-2">
                   {note.tags.map(tag => (
                     <span key={tag} className="px-3 py-1 rounded-lg bg-white/5 text-[10px] font-bold text-zinc-500 uppercase">#{tag}</span>
                   ))}
                </div>
              )}
            </div>
          ...
          <aside className="space-y-6">
            <div className="rounded-[2rem] border border-white/5 bg-white/5 p-8 backdrop-blur-xl sticky top-32">
               <div className="flex items-center gap-4 mb-8">
                 ...
                 <div>
                    <Link href={`/profile/${note.author.username}`} className="text-white font-bold hover:text-indigo-400 transition-colors">{note.author.name}</Link>
               ...
               <div className="space-y-3">
                 <div className="flex gap-2">
                   ...
                   <button 
                    ...
                    className={`aspect-square w-14 rounded-2xl border flex items-center justify-center transition-all active:scale-95 ${isLiked ? 'bg-rose-500/10 border-rose-500/50 text-rose-500 shadow-lg shadow-rose-500/10' : 'bg-white/5 border-white/10 text-zinc-400 hover:text-white hover:bg-white/10'}`}
                   >
                     <Heart className={`w-5 h-5 ${isLiked ? 'fill-current' : ''}`} />
                   </button>
                   <button onClick={handleShare} className="aspect-square w-14 rounded-2xl border border-white/10 bg-white/5 text-zinc-400 flex items-center justify-center hover:text-white hover:bg-white/10 transition-all active:scale-95">
                     <Share2 className="w-5 h-5" />
                   </button>
                 </div>
                 <button 
                  onClick={handleToggleSave}
                  className={`w-full py-4 rounded-2xl border flex items-center justify-center gap-2 text-[10px] font-black uppercase tracking-widest transition-all active:scale-95 ${isSaved ? 'bg-indigo-600/10 border-indigo-500/50 text-indigo-400' : 'bg-white/5 border-white/10 text-white hover:bg-white/10'}`}
                 >
